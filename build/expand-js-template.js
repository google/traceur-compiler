...
for (var i = 2; i < process.argv.length; i++) {
  if (options) {
    console.error('Either --nolint or --deps may be specified.');
    process.exit(1);
  }
  if (!(match = process.argv[i].match(/--(\w+)(?:=(.*))?/))) {
    break;
  }
  switch (match[1]) {
    case 'nolint':
      var r = new RegExp(match[2]);
      var fmt = format('// #line 0 "%%s" auto generated by %s\n%%s',
                       path.basename(process.argv[1]));
      options = {
        pre: function(file, outDir, outStream) {
          if (r.test(file))
            outStream.write(format(fmt, 'nolint', '//:nolint\n'));
        },
        visit: nop,
        post: function(file, outDir, outStream) {
          if (r.test(file))
    ...
    case 'deps':
      ...
      expandFile(process.argv[i + 1], '.', {write: nop}, options);
      process.exit(0);
    default:
      options = {};
  }
